...
interface ExamActions {
  ...
  setChecking: (checking: boolean) => void; // NEW: For check answer loading state
  setError: (error: string | null) => void;

  // NEW: Check Results Management
  setQuestionCheckResult: (
    questionId: string,
...
const initialState: ExamState = {
  ...
  questionsStatus: {},
  questionsAnswers: {},
  questionsCheckResults: {}, // NEW: Initialize check results
  timePerQuestion: 0,
  remainingTime: 0,
  isTimerActive: false,
  isLoading: false,
  isSubmitting: false,
  isChecking: false, // NEW: Initialize checking state
  error: null,
  isPaused: false,
  isFinished: false,
};
...
export const useExamStore = create<ExamState & ExamActions>()(
  subscribeWithSelector((set, get) => ({
    ...
    initializeExam: (examData) => {
      ...
      const questionsCheckResults: Record<string, QuestionCheckResult> = {}; // NEW
      ...
      // FIXED: Handle both string and number types for remaining time
      const remainingTimeValue =
        examData.current_question?.remaining || examData.remaining;
      const parsedRemainingTime =
        typeof remainingTimeValue === "number"
          ? remainingTimeValue
          : parseInt(String(remainingTimeValue) || "0");
    ...
    // NEW: Set checking state
    setChecking: (checking) => {
      console.log("🟡 [STORE] Checking:", checking);
    ...
    // NEW: Set question check result
    setQuestionCheckResult: (questionId, result) => {
      console.log(
        "🟢 [STORE] Setting check result for question:",
        questionId,
        result
      );
      set((state) => ({
        questionsCheckResults: {
          ...state.questionsCheckResults,
          [questionId]: result,
        },
      }));
    },
...
export const useExamSelectors = () => {
  ...
  // NEW: Get current question check result
  const currentQuestionCheckResult = store.currentQuestion
    ? store.questionsCheckResults[store.currentQuestion.id]
    : undefined;
  ...
  if (store.currentQuestion) {
    console.log("🔍 [SELECTORS] Current question state:", {
      ...
      checkResult: currentQuestionCheckResult,
    });
  }
  ...
  return {
    ...
    currentAnswer,
    currentQuestionCheckResult, // NEW: Add check result to selectors

    // UI state
    isLoading: store.isLoading,
    isSubmitting: store.isSubmitting,
    isChecking: store.isChecking, // NEW: Add checking state
    error: store.error,
