...
import { getPowerPointBySlug } from "@/lib/api/powerpoints.service";
import { qk } from "@/lib/queryKeys";

export default function PowerPointContent({
  powerSlug,
}: {
  powerSlug?: string;
}) {
  const enabled = Boolean(powerSlug);
  const { data, isLoading, isError, error } = useQuery({
    queryKey: enabled
      ? qk.powerpoints.item(powerSlug as string)
      : ["powerpoints", "idle"],
    queryFn: ({ signal }) => getPowerPointBySlug(powerSlug as string, signal),
    enabled,
    staleTime: 60_000,
  });

  const title = (data as any)?.title || powerSlug;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm mt-1">
              No content found for this power point.
            </p>
          </div>
        </div>
      </div>
