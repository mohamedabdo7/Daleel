...
// --- Simple icons (kept as in your original file) ---
...
export interface SidebarItem {
  id: string; // we use the category/article SLUG here
  title: string;
...
export interface CreativeSidebarProps {
  title: string;
  items: SidebarItem[];
  onItemClick: (item: SidebarItem) => void; // fired for child clicks
  activeItemId?: string; // active article slug
  className?: string;
  isOpen?: boolean;
  onClose?: () => void;
  onCategoryToggle?: (categoryId: string, willExpand: boolean) => void; // NEW
  loadingCategoryId?: string | null; // NEW
}
...
const CreativeSidebar: React.FC<CreativeSidebarProps> = ({
  ...
  const toggleSection = (sectionId: string) => {
    ...
    const willExpand = !newExpanded.has(sectionId);
    if (willExpand) newExpanded.add(sectionId);
    else newExpanded.delete(sectionId);
    setExpandedSections(newExpanded);
    onCategoryToggle?.(sectionId, willExpand);
  };

  const getIcon = (itemId: string) => {
    const iconMap: Record<string, React.ReactNode> = {
      "section-1-family-medicine": <Stethoscope className="w-5 h-5" />,
      pediatrics: <Baby className="w-5 h-5" />,
      "section-2-internal-medicine": <Heart className="w-5 h-5" />,
  ...
  const SidebarItemComponent: React.FC<SidebarItemProps> = ({
    ...
    const isActive = activeItemId === item.id; // for child highlighting
            ...
            {isParentSection && (
              <div className="flex items-center gap-2">
                {loadingCategoryId === item.id && (
                  <span className="inline-block w-3 h-3 rounded-full border-2 border-[var(--primary-color)] border-t-transparent animate-spin" />
                )}
                <div
  ...
  return (
    <>
      {/* Toggle button kept for mobile usage */}
      <button
        onClick={() => onClose && onClose()}
        className={`fixed top-4 left-4 z-[60] lg:hidden p-3 rounded-xl bg-white shadow-lg border border-gray-200 hover:bg-[var(--primary-light)] hover:border-[var(--primary-color)] transition-all duration-300 group ${
      ...
      <aside
        ...
        style={{
          ...
          ["--primary-color" as any]: "#1D4671",
          ["--primary-dark" as any]: "#153654",
          ["--primary-light" as any]: "#E8F2FF",
          ["--primary-lighter" as any]: "#F5F9FF",
        }}
      >
        <div className="sticky top-0 bg-white/80 backdrop-blur-lg border-b border-gray-200/60 p-6 z-20">
          <div className="flex items-center space-x-3">
            <div className="p-2 rounded-xl bg-gradient-to-br from-[var(--primary-color)] to-[var(--primary-dark)]">
              {/* decorative icon */}
              <svg
