...
export default function ArticlesPage() {
  ...
  const selectedArticle = searchParams.get("sec") || undefined;
  ...
  const sidebarItems = useMemo<SidebarItem[]>(
    () =>
      (categories || []).map((c) => ({
        id: c.slug,
        title: c.title,
        icon: BookOpen, // Custom icon for article categories
      })),
    [categories]
  );
  ...
  const sidebarConfig: SidebarConfig = {
    ...
    subtitle: "Family medicine guide",
    ...
    try {
      ...
      if (itemIndex !== -1) {
        sidebarItems[itemIndex].children = (list || []).map((a) => ({
          id: a.slug,
          title: a.title,
          icon: FileText, // Icon for individual articles
        }));
      }
      ...
      <div className="flex relative">
        ...
        <aside className="hidden lg:block w-80 flex-shrink-0">
          {catsLoading ? (
            ...
            <GenericSidebar
              items={sidebarItems}
              selectedItemId={selectedArticle}
        ...
        <div className="lg:hidden fixed bottom-4 right-4 z-[55]">
          <button
            ...
            className="bg-[#0A2C75] text-white p-3 rounded-full shadow-lg hover:bg-[#0A2C75]/90 transition-colors"
