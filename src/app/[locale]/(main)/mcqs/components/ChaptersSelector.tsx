...
export const ChaptersSelector: React.FC<ChaptersSelectorProps> = ({
  ...
  const watchedValues = useWatch({
    control,
    // Removed chapters_type dependency entirely
    name: ["section_id", "question_type", "chapters"],
  });

  const [sectionId, questionType, selectedChapters] = watchedValues as [
    number | undefined,
  ...
  const {
    ...
    sectionId as number | undefined,
    questionType as QuestionType
  );

  // Calculate max questions only from selected chapters
  React.useEffect(() => {
  ...
  }, [chapters, selectedChapters, onMaxQuestionsChange]);

  // Don't show anything if no section selected
  if (!sectionId) {
    return null;
  }
  ...
  if (chapters.length === 0) {
    return (
      <FormItem>
        <FormLabel>Available Chapters</FormLabel>
  ...
  // Always show the chapter selection UI (no chapters_type dependency)
  return (
    <FormField
      control={control}
      name="chapters"
            ...
            <ScrollArea className="h-64 w-full border rounded-md p-4">
              <div className="space-y-3">
                {chapters.map((chapter) => {
                  const isChecked = field.value?.includes(chapter.id) || false;
                  return (
                    <div
                      key={chapter.id}
