      ...
      <motion.div variants={cardVariants} className="max-w-7xl mx-auto">
        ...
        <div className="block sm:hidden">
          <div className="space-y-4">
            {latestNews.slice(start, start + 1).map((item, idx) => (
              <motion.div
                key={`${item.id}_${item.file}_${start}`}
                className="bg-[#F3F8FE] rounded-2xl w-full h-[320px] flex flex-col shadow-sm overflow-hidden"
          ...
          <div className="flex justify-center items-center gap-4 mt-6">
            <motion.button
              ...
              className="rounded-full bg-white shadow-lg border border-[#E6F0FA] text-[#1976D2] 
                       ...
                       disabled:opacity-30 disabled:pointer-events-none backdrop-blur-sm"
              aria-label="Previous"
            ...
            <span className="text-sm text-gray-600">
              {start + 1} / {latestNews.length}
            </span>

            <motion.button
              onClick={handleNext}
              disabled={start >= latestNews.length - 1}
              className="rounded-full bg-white shadow-lg border border-[#E6F0FA] text-[#1976D2]
                       w-12 h-12 flex items-center justify-center
                       disabled:opacity-30 disabled:pointer-events-none backdrop-blur-sm"
              aria-label="Next"
              ...
              initial="hidden"
              animate="visible"
              whileHover={start >= latestNews.length - 1 ? {} : "hover"}
              whileTap={start >= latestNews.length - 1 ? {} : "tap"}
            >
        ...
        <div className="hidden sm:block">
          <div className="flex justify-center items-center gap-3 sm:gap-6">
            ...
            <motion.button
              ...
              initial="hidden"
              animate={
                start >= latestNews.length - cardsVisible
                  ? "disabled"
                  : "visible"
              }
              whileHover={
