...
const TestimonialSection: React.FC<TestimonialSectionProps> = ({ reviews }) => {
  ...
  const ReviewAvatar = ({ item }: { item: Review }) => {
    ...
    // Show fallback for invalid URLs or errors
        ...
        <div className="block sm:hidden">
          <div className="space-y-6">
            {visibleReviews.map((item, idx) => (
              <motion.div
                key={`${item.id}_${item.file}_${currentPage}`}
                className="bg-[#E6F3FE] rounded-3xl w-full min-h-[340px] 
          ...
          <div className="flex justify-center items-center gap-4 mt-6">
            <motion.button
              ...
              className="rounded-full bg-white shadow-md border border-[#E6F0FA] text-[#1976D2] 
                       hover:bg-[#E6F0FA] transition w-12 h-12 flex items-center justify-center
